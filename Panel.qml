...
Panel {
  id: root
  moduleName: "yeleticc.vpn"
  ipcTarget: "yeleticc.vpn"
  manageIpc: false
  ...
  readonly property bool switchHasCursor: headerHasCursor && headerIndex === 2
  ...
  readonly property string statusLine: {
    ...
    if (!backend) {
      ...
      return vpn.setupHint !== ""
        ? vpn.setupHint
        : "Drop a WireGuard *.conf in the profiles directory to use this widget."
    }
    if (backend.actionStatus !== "") return backend.actionStatus
    if (backend.lastError !== "") return backend.lastError
    return backend.healthWarning
  }

  function ensureCursor() {
    if (rows.length === 0 && focusSection === "rows") {
      if (providersOpen || !backend) focusSection = "header"
    }
    ...
    else if (!reconnectButton.visible && headerIndex === 1) headerIndex = 2
    if (headerIndex < 0) headerIndex = 0
    if (headerIndex > 2) headerIndex = 2
  ...
  function moveCursor(dx, dy) {
    ...
    if (focusSection === "hero") {
      ...
      else if (rows.length > 0 || (!providersOpen && backend)) setRowCursor(0)
      return
    }
  ...
  function stepHeader(direction) {
    ...
    var options = reconnectButton.visible ? [0, 1, 2] : [0, 2]
    var current = options.indexOf(headerIndex)
    if (current < 0) current = 0
    headerIndex = options[Math.max(0, Math.min(options.length - 1, current + direction))]
  }
  ...
  function activateCursor() {
    ...
    if (focusSection === "header") {
      ...
      else if (headerIndex === 1 && backend) vpn.reconnectActive()
    ...
    if (focusSection === "rows") {
      ...
      } else if (!providersOpen && backend && typeof backend.openProfilesFolder === "function") {
        backend.openProfilesFolder()
      }
      return
    }
    if (!backend) return
  ...
  // keyboard — wg-quick's root prompt via pkexec showing a polkit dialog.
  ...
  IpcHandler {
    ...
    function connect(target: string): string {
      ...
      // A bare profile name is the documented shorthand, and it is the row key
      // that spells it the same way for every target.
        ...
        Column {
          ...
          Item {
            ...
            readonly property real controlsWidth: gearButton.width
              + (reconnectButton.visible ? reconnectButton.width + Style.space(8) : 0)
            ...
            PanelActionButton {
              id: gearButton
              anchors.right: reconnectButton.visible ? reconnectButton.left : (masterSwitch.visible ? masterSwitch.left : parent.right)
              anchors.rightMargin: (reconnectButton.visible || masterSwitch.visible) ? Style.space(8) : 0
              anchors.verticalCenter: parent.verticalCenter
            ...
            PanelActionButton {
              ...
              anchors.verticalCenter: parent.verticalCenter
              visible: root.backend !== null && root.backend.connected
              iconText: Model.GLYPH_REFRESH
              tooltipText: "Reconnect"
              hasCursor: root.headerHasCursor && root.headerIndex === 1
              foreground: root.foreground
              fontFamily: root.fontFamily
              onHovered: function(on) { if (on) root.setHeaderCursor(1) }
              onClicked: vpn.reconnectActive()
            }
          ...
          CursorSurface {
            ...
            PanelHero {
              ...
              meta: root.backend ? (root.backend.connected ? "● " + root.backend.summary + " · Connected" : root.backend.summary) : "Nothing detected"
              foreground: (root.backend && root.backend.connected) ? "#22c55e" : root.foreground
              fontFamily: root.fontFamily
              iconOpacity: vpn.anyConnected ? 1.0 : 0.5
              iconComponent: Component {
                Text {
                  text: root.backend ? root.backend.glyph : Model.GLYPH_VPN
                  color: vpn.anyConnected ? "#22c55e" : root.dim
                  font.family: root.fontFamily
          ...
          PanelSeparator {
            visible: !root.providersOpen && root.backend !== null && root.backend.connected
            foreground: root.foreground
          }
          ...
          Column {
            ...
            Grid {
              ...
              StatCard {
                ...
                iconText: Model.GLYPH_DOWNLOAD
                accentColor: "#22c55e"
                title: "RECEIVING"
                value: Model.formatRate(root.backend ? root.backend.rxRate : 0)
                subtext: "↓ " + Model.formatBytes(root.backend ? root.backend.rxBytes : 0)
                fontFamily: root.fontFamily
              }

              StatCard {
                width: statsGrid.cellWidth
                iconText: Model.GLYPH_UPLOAD
                accentColor: "#38bdf8"
                title: "SENDING"
                value: Model.formatRate(root.backend ? root.backend.txRate : 0)
                subtext: "↑ " + Model.formatBytes(root.backend ? root.backend.txBytes : 0)
                fontFamily: root.fontFamily
              }

              StatCard {
                width: statsGrid.cellWidth
                iconText: Model.GLYPH_SPEED
                accentColor: (root.backend && root.backend.pingInfo && root.backend.pingInfo.packetLoss > 0) ? "#ef4444" : "#f59e0b"
                title: "LATENCY"
                value: Model.formatLatency(root.backend ? root.backend.pingInfo : null)
                subtext: "Loss: " + Model.formatPacketLoss(root.backend ? root.backend.pingInfo : null)
                fontFamily: root.fontFamily
              }

              StatCard {
                width: statsGrid.cellWidth
                iconText: Model.GLYPH_LOCK
                accentColor: "#a855f7"
                title: "TUNNEL"
          ...
          Column {
            ...
            PanelSectionHeader {
              text: {
                ...
                return root.filterVisible && root.backend && root.backend.filter !== "" ? "MATCHING" : "PROFILES"
              }
              foreground: root.foreground
              fontFamily: root.fontFamily
            }
            ...
            Text {
              visible: root.rows.length === 0 && root.providersOpen
              width: parent.width
              text: "No VPN tool detected on this machine."
              color: root.dim
              font.family: root.fontFamily
              font.pixelSize: Style.font.bodySmall
            ...
            Column {
              ...
              Text {
                width: parent.width
                text: root.backend ? root.backend.emptyText : "No profiles found."
                color: root.dim
                font.family: root.fontFamily
                font.pixelSize: Style.font.bodySmall
                horizontalAlignment: Text.AlignHCenter
  ...
  component TargetRow: CursorSurface {
    ...
    RowLayout {
      ...
      Text {
        ...
        color: targetRow.isCurrent ? "#22c55e" : (targetRow.rowMuted ? root.dim : root.foreground)
        font.family: root.fontFamily
        font.pixelSize: Style.font.icon
        Layout.alignment: Qt.AlignVCenter
      }
      ...
      ColumnLayout {
        ...
        Text {
          ...
          font.family: root.fontFamily
          font.pixelSize: Style.font.body
          font.weight: targetRow.isCurrent ? Font.DemiBold : Font.Normal
          elide: Text.ElideRight
        }
        ...
        Text {
          ...
          color: targetRow.isCurrent ? "#22c55e" : root.dim
          font.family: root.fontFamily
  ...
  component StatCard: Rectangle {
    ...
    color: Util.alpha(accentColor, 0.08)
    border.width: 1
    border.color: Util.alpha(accentColor, 0.26)
    ...
    Column {
      ...
      Row {
        ...
        Text {
          ...
          font.family: statCard.fontFamily
          font.pixelSize: Style.font.icon
          anchors.verticalCenter: parent.verticalCenter
        }
        Text {
