...
Item {
  ...
  readonly property var backends: [wireGuard]
  ...
  readonly property string barSummary: {
    ...
    if (!backend) {
      if (active && active.profiles && active.profiles.length === 0) return "No profiles"
  ...
  function reconnectActive() {
    var backend = root.active
    if (!backend || !backend.connected || backend.busy) return
    cancelPending()
    clearNotice()
    backend.reconnect()
  }
